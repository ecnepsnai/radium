...
func TestUtilSanitizePath(t *testing.T) {
	...
	test("MLP:_Stable_Relationship", "MLP_Stable_Relationship")
	test("Café Exposé", "Café_Exposé")
	test("Pony ★ Academy - Chapter 2 : Magic Lessons", "Pony_★_Academy_-_Chapter_2__Magic_Lessons")
	test("CinderFrost, Chapter #3", "CinderFrost_Chapter_3")
	test("../../../../../../etc/passwd", "_............etcpasswd")
	test("\\\\172.16.1.2\\Vault", "172.16.1.2Vault")
	test("blah\000", "blah")
}
...
func TestUtilTouchDirectory(t *testing.T) {
	...
	if TimeEquals(subdirMtime, getMTime(subdir)) {
		t.Errorf("Mtime did not change for sub directory when expected. Before touch: %s After touch: %s", subdirMtime.String(), getMTime(subdir).String())
	}
