...
gulp.task('js', gulp.parallel(function(done) {
    ...
    return gulp.src('./js/ng/**/*.js')
        ...
        .pipe(gulp.dest(BUILD_DIRECTORY_BASE + '/static/js'));
}, function(done) {
    'use strict';
...
gulp.task('html', gulp.parallel(function(done) {
    ...
    return gulp.src(['./js/ng/pages/**/*.html', './js/ng/components/**/*.html'])
        ...
        .pipe(rename({ dirname: '' }))
        .pipe(gulp.dest(BUILD_DIRECTORY_BASE + '/static/html/'));
}, function(done) {
    'use strict';
...
gulp.task('css', gulp.parallel(function(done) {
    ...
    return gulp.src(['./fonts/*.css'])
        ...
        .pipe(gulp.dest(BUILD_DIRECTORY_BASE + '/static/css'));
...
gulp.task('img', function() {
    ...
    return gulp.src('./img/**/*')
        .pipe(gulp.dest(BUILD_DIRECTORY_BASE + '/static/img'));
});
...
gulp.task('fonts', function() {
    ...
    return gulp.src(['./fonts/*.eot', './fonts/*.svg', './fonts/*.ttf', './fonts/*.woff', './fonts/*.woff2'])
        .pipe(gulp.dest(BUILD_DIRECTORY_BASE + '/static/fonts'));
});
...
gulp.task('watch', gulp.parallel(
    function(done) {
        ...
        gulp.watch(['./html/*.html', './js/ng/pages/**/*.html', './js/ng/components/**/*.html'], gulp.series('html'));
...
gulp.task('release', gulp.parallel(function(done) {
    ...
    return gulp.src([BUILD_DIRECTORY_BASE + '/static/js/*.js', '!' + BUILD_DIRECTORY_BASE + '/static/js/*min.js'])
        .pipe(strip())
        .pipe(minify({
            ext: {
                min: '.js'
    ...
    return gulp.src(BUILD_DIRECTORY_BASE + '/static/**/*.map', { read: false })
        .pipe(clean({ force: true }));
}));
